...
:: set o_fol to the folder that Office is in
:: Currently is is downloaded into "C2R_Monthly"
set o_fol=C2R_Monthly

if exist "!_work!\%o_fol%\Office\Data\*.cab" (
for /f %%# in ('dir /b /ad "!_work!\%o_fol%\Office\Data\" 2^>nul') do if exist "!_work!\%o_fol%\Office\Data\%%#\stream*.dat" (
  set "CTRsource=%~dp0%o_fol%\"
  )
)
...
echo.
echo %line%
REM choice /c 123X /n /m "Choose a menu option to proceed, or press X to exit: "
goto :Menu21Apps
...
if not %_Teams%==0 echo. T. Microsoft Teams       : %_Teams%
echo %line%
REM choice /c AENOPRSWD567890TX /n /m "Change a menu option, press 0 to proceed, 9 to go back, or X to exit: "
goto :Menu21App2
set errortmp=%errorlevel%
if %errortmp%==17 goto :eof
...
echo %line%
REM choice /c 1234567809X /n /m "Choose a menu option to proceed, press 9 to go back, or X to exit: "
set inpt=3&goto :MenuChn
...
echo %line%
REM choice /c 123456709X /n /m "Change a menu option, press 0 to proceed, 9 to go back, or X to exit: "
goto :MenuFinal
...
echo.
echo %line%
REM choice /c 1239X /n /m "Choose a menu option to proceed, press 9 to go back, or X to exit: "
set _unattend=True&goto :MenuFinal2
if errorlevel 5 goto :eof
...
echo SKUs=
echo ExcludedApps=
echo UpdatesEnabled=True
echo AcceptEULA=True
echo PinIconsToTaskbar=False
echo ForceAppShutdown=True
echo AutoActivate=False
echo DisableTelemetry=True
echo DisplayLevel=True
echo AutoInstallation=True
